...
with open("timp_interclasare_crescator.csv","a",newline="") as f2:
    writer=csv.writer(f2)
    header=["Nr. el.","Timp","Comp.","mutari"]
    writer.writerow(header)

def merge_crescator(stanga, dreapta):
...
def merge_sort_crescator(l):
    if len(l) <= 1:
      ...
      # recursivitate
      jumatatea_stanga = merge_sort_crescator(jumatatea_stanga)
      jumatatea_dreapta = merge_sort_crescator(jumatatea_dreapta)

      # Merge
      return merge_crescator(jumatatea_stanga,jumatatea_dreapta)
...
with open("date_testare.txt","r") as f2:
    for line in f2:
        ...
        n_comparatii=0
        n_mutari=0
        merge_sort_crescator(l)
        stop_timp=time.time()
        timp=stop_timp-start_timp
        with open("timp_interclasare_crescator.csv","a") as f2:
            writer=csv.writer(f2)
