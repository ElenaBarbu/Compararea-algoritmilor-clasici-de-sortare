...
def quicksort_descrescator(x, stanga, dreapta):
    if stanga < dreapta:
        q = partitie(x, stanga, dreapta)
        x[stanga:q] = quicksort_descrescator(x, stanga, q - 1)
        x[q + 1:dreapta + 1] = quicksort_descrescator(x, q + 1, dreapta)
    return x[stanga:dreapta + 1]



with open("timp_quick_descrescator.csv","a",newline="") as f2:
...
with open("date_testare.txt","r") as f2:
    for line in f2:
        ...
        n_comparatii=0
        n_mutari=0
        quicksort_descrescator(l,0, len(l)-1)
        stop_timp=time.time()
        timp=stop_timp-start_timp
        with open("timp_quick_descrescator.csv","a") as f2:
            writer=csv.writer(f2)
